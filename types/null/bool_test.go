...
func TestBoolMarshalText(t *testing.T) {
	...
	// Null NullBools should be encoded as an empty string.
	nul := null.NullBool{}
...
func TestBoolUnmarshalText(t *testing.T) {
	...
	var zero null.NullBool
	err = zero.UnmarshalText([]byte("false"))
	fatalIf(t, err, FileLine())
	assertBool(t, false, zero, FileLine())

	// Successful Null Parses

	var blank null.NullBool
	err = blank.UnmarshalText([]byte(""))
	fatalIf(t, err, FileLine())
	assertNullBool(t, blank, FileLine())
	...
	var booStr null.NullBool
	// Booleans wrapped in quotes aren't booleans.
	err = booStr.UnmarshalText(boolStringJSON)
	fatalUnless(t, err, FileLine())

	var number null.NullBool
	err = number.UnmarshalText([]byte("1"))
	fatalUnless(t, err, FileLine())
